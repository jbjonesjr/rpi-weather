...
let fetch_current_conditions = () => {
  try {
    ...
    return client.query(`SELECT observed_at as "time", TRUNC(temperature_f::numeric,2) as "temp_f", wind_kph as "wind", wind_dir_deg as wind_dir, rain_diff_mm as "rain", humidity FROM reports WHERE  NOT EXISTS (SELECT * FROM reports AS reports_temp WHERE reports.sensor_id = reports_temp.sensor_id AND reports.observed_at < reports_temp.observed_at) and sensor_id = '2';`)
      .then(result => {
