...
SELECT (observed_at at time zone 'America/New_York') as "time", 
        ...
        date_trunc('day', observed_at::date) = date_trunc('day', (now())::date)
        order by observed_at desc limit 1
        

        select * from sensors;

        update sensors set data_validity = 0 where pid >= 4;
        delete from sensors where pid >= 4 and sensor_id = '715591';